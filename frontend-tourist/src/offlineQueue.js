// Offline queue-and-retry for booking/order attempts. When a POST to
// /api/bookings or /api/orders fails because the device has no signal
// (the common case for a tourist at sea or on a remote island — not
// because the request was invalid), it's saved here instead of just
// failing, and automatically retried the moment the browser regains
// connectivity. localStorage rather than IndexedDB — the payloads are
// small, and this needs to work from plain synchronous code in
// api/client.js without every caller becoming async-around-a-DB-open.

const STORAGE_KEY = 'atollisle_offline_queue';
